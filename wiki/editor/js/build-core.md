储存解释时用的标签表和属性表以及构建页面所需的函数

其中
- globalAttributeList 为全局属性表
- attributeList 为普通属性表
- tagList 为标签表
- 下面这几个函数涉及到构建页面时的处理方式，所以他们的用法记录在 ./chunk/route/buildProject.js.md 中
  - tab
  - build_tree
  - build_attribute
# 属性表
...
```js
{
    ...
    }//当该元素编辑修改时执行的函数，在系统执行modify常规操作后执行
}
```
# 标签表
```js
...
- tree(可选) : 构建页面时的处理方法，具体请查看 buildProject.md
